-- 32vid-decode.lua
-- Module form of MCJack123's 32vid-player.lua (from the sanjuuni project,
-- https://github.com/MCJack123/sanjuuni, MIT licensed). The binary parsing,
-- Huffman/DEFLATE decoding, and frame-unpacking logic below is copied
-- verbatim from the upstream player -- do not "clean up" or rewrite it by
-- hand, it's exact byte-format-dependent code with no independent way to
-- verify correctness outside a real CC:Tweaked runtime. Only the entry point
-- was changed: instead of opening its own file from a shell argument and
-- immediately playing it, M.decode(file) takes an already-open read handle
-- and returns the parsed video/audio/subtitle data so a caller can add its
-- own playback loop (pause, volume, progress UI, chunk chaining, etc).
...
-- { width, height, fps, video, audio, subtitles }. video/audio/subtitles are
-- the same in-memory tables the stock 32vid-player.lua builds (frames of
-- term.blit-ready strings + a 16-entry RGB palette per video frame; PCM
-- sample tables or DFPWM-decoded buffers per audio chunk); rendering and
-- audio playback are left to the caller. Closes `file` when done.
function M.decode(file, onProgress)
    ...
    for _ = 1, nStreams do
        ...
        if frameType == 0 and not video then
            ...
            local pos = 1
            for i = 1, nFrames do
                if onProgress and (i % 20 == 0 or i >= nFrames - 2) then
                ...
                for y = 1, height do
                    local line = {"", "", "", {}}
                    for x = 1, width do
                        if pos + 5 + 1 >= #data then error(("32vid decode ran out of data: frame %d, x=%d y=%d"):format(i, x, y)) end
                        local n = readField()
    ...
    file.close()
    if not video then error("No video stream found in 32vid chunk") end
