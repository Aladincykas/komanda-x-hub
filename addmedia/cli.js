...
async function convertVideoLocally(inputPath, workDir, baseName, opts) {
    ...
    const segments = fs.readdirSync(workDir).filter((f) => /^seg_\d+\.mp4$/.test(f)).sort();
    if (segments.length === 0) throw new Error("ffmpeg produced no segments -- check the input file.");
...
async function addVideoFlow(ctx) {
    const inputPath = await ask("Path to a video file (mp4, mkv, etc): ");
    if (!inputPath) { console.log("Nothing entered, exiting."); return; }
    if (!fs.existsSync(inputPath)) throw new Error(`File not found: ${inputPath}`);

    const detectedTitle = path.basename(inputPath, path.extname(inputPath));
    const customName = await ask(`Name for your library [${detectedTitle}]: `);
    const title = customName || detectedTitle;
    const slug = slugify(title);

    const sizeAnswer = await ask(`Monitor size in characters [${DEFAULT_CHAR_WIDTH}x${DEFAULT_CHAR_HEIGHT}]: `);
    let charWidth = DEFAULT_CHAR_WIDTH, charHeight = DEFAULT_CHAR_HEIGHT;
    const sizeMatch = sizeAnswer.match(/^(\d+)x(\d+)$/i);
    if (sizeMatch) { charWidth = parseInt(sizeMatch[1], 10); charHeight = parseInt(sizeMatch[2], 10); }

    const segAnswer = await ask(`Segment length in seconds [${DEFAULT_SEGMENT_SECONDS}]: `);
    const segmentSeconds = parseInt(segAnswer, 10) || DEFAULT_SEGMENT_SECONDS;

    const workDir = fs.mkdtempSync(path.join(os.tmpdir(), "addmedia_video_"));
    let chunkCount, fps, durationSec;
    try {
        ({ chunkCount, fps, durationSec } = await convertVideoLocally(inputPath, workDir, slug, { charWidth, charHeight, segmentSeconds }));
